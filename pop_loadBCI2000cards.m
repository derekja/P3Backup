function EEG = pop_loadBCI2000(fileName, events, correctcardsin)
...
elseif nargin >= 1 %input argument may be cell array of fileName(s) or a string
    if iscell(fileName)
...
elseif nargin>1%No GUI, ignore events provided.
    events = stateNames; %Select all events.

    if ~isempty(correctcardsin)
        correctcards = eval(correctcardsin);
    end
end

    
% put type of game switch here
% error check return
%temp2 = inputgui('geometry', {1, 1, 1}, 'geomvert', [1, 5, 1], ...
%        'uilist', {{'style', 'edit', 'string', 'card rounds'}, ...
%        {'style', 'edit', 'string', 'card rounds2'}, ...
%        {'style', 'text', 'string', 'number 3'}});
...
StimTypeData = zeros(file_samples,1,'uint8');
stateNames = fieldnames(states);


StimTypeData = states.('StimulusType');

cardgame = 0;
if (max(StimTypeData)==0)
    cardgame = 1;
end


if (cardgame == 1)
    TargetData = states.('SelectedTarget');
    TargetTimes = find([false;diff(TargetData) ~= 0]);
    TargetValues = TargetData(TargetTimes);
    StimCodeData = states.('StimulusCode');

    for j = TargetTimes(1):length(TargetData) % starting after 1st target (memorize cards...)
   
        for k = 1:length(TargetTimes) % yeah I know this is innefficient as hell
            if (j < TargetTimes(k))
                TargetNow = TargetValues(k); %old method to assume correct
                TargetNow = correctcards{k}; %pulling actual correct in from command line
                break;
            end
        end
    
        if ((StimCodeData(j) > 0) && (StimCodeData(j) == TargetNow))
            StimTypeData(j) = 1;
        end
    ...
    for k = 1:length(stateNames)
        if ismember(stateNames{k}, 'StimulusType')
            StimType = k;
            StimTypeData = states.(stateNames{StimType});
            break
        end
    end
...
%select non-overlapping windows by running through time and throwing away
%StimulusBegin other than at .8 sec time boundaries
% not doing yet obviously



%% Convert states to compressed EEGLAB event format
states = compressData(states, StimTypeData);
stateNames = fieldnames(states);
for k = 1:length(stateNames)
    if ismember(stateNames{k}, 'StimulusBegin')
...
function data = compressData(data, StimTypeData)
...
if isstruct(data)
    ...
    for k = 1:length(stateNames)
        if ismember(stateNames{k}, compressStates)  % If the event is in the list of known events, compress it
            if ~isstruct(data.(stateNames{k}))
                data.(stateNames{k}) = compressData(data.(stateNames{k}), StimTypeData);
            end
            ...
            if length(U) > 1 && length(U) < length(value)*relativeEventLength
                data.(stateNames{k}) = compressData(value, StimTypeData);
            else
    ...
    if exist('StimTypeData', 'var')
        st = StimTypeData(ev_ch_ix);
    else
        st = zeros(size(data(ev_ch_ix)));
    end
    ...
    data.StimulusType = st(keep_event);
end
